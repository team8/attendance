...
def makeNewStudent(ID):
    ...
    name = re.search(r'<input name="name" type="hidden" label="name" value=(.*?)"',html)
    if name is None:
        return None
    culledName = name.group(1)
    Student(name=culledName,studentID=ID,subteam=Subteam.objects.get(name="Unknown")).save()
    return True
    #return False
