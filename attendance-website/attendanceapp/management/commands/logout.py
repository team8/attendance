...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        for person in Student.objects.all():
            if person.atLab:
                logOut(person)
                CLIENT.api_call("im.open", user="U039ZJW8K", return_im=True)
    def logOut(student):
        ...
        timeWorked=HoursWorked(timeIn=lastLoggedIn,day = timeNow.strftime("%A"),timeOut=timeNow, totalTime=hours_elapsed.total_seconds(), validTime=valid_hours_elapsed.total_seconds(), autoLogout=True, percentTime = percentTime, weight = weight, owner = student)
        timeWorked.save()
